...
def CustomLoss(y_true, y_pred_logits, y_pred_bbox):
    """
    ...
    for i in range(batch):
        out_cls_loss = -y_pred_logits[i][:,y_true[i][:,-1]]
        ...
        cls_loss = tf.losses.sparse_categorical_crossentropy(y_true[i][:,-1][:,np.newaxis], y_pred_logits[i,row_ind])
        box_loss = tf.losses.mean_absolute_error(y_true[i][:,:-1], y_pred_bbox[i,row_ind])
